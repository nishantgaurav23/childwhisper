...
- Run the `/verify-spec` workflow:
  - Code exists, tests pass, lint clean, outcomes met
- If verification FAILS: fix the issues and re-verify

### Step 5: Generate Explanation
- Run the `/explain-spec` workflow:
  - Create `explanation.md` in the spec folder documenting why, what, how, connections
...
## Rules
- STOP immediately if dependencies are not met
