"""Whisper-large-v3 LoRA fine-tuning script for children's speech.

Loads config from YAML, sets up model with INT8 quantization + LoRA adapter,
SpecAugment + gradient checkpointing, trains with Seq2SeqTrainer, evaluates
with WER, and optionally pushes adapter to HF Hub.
...
def load_training_config(config_path: str) -> dict:
    """Load training config from YAML, merging common + whisper_large_v3 sections."""
    ...
    if "whisper_large_v3" in raw:
        config.update(raw["whisper_large_v3"])
    return config


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse CLI arguments for LoRA training."""
    parser = argparse.ArgumentParser(
        description="Fine-tune Whisper-large-v3 with LoRA on children's speech"
    )
    parser.add_argument(
        "--metadata-path", type=str, required=True, help="Path to metadata JSONL file"
    )
    parser.add_argument(
        "--audio-dir", type=str, required=True, help="Directory containing audio files"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./checkpoints/whisper-large-v3-lora",
...
def create_lora_config(config: dict) -> LoraConfig:
    """Create a LoraConfig from the training config dict."""
...
def setup_model(config: dict) -> tuple:
    """Load Whisper-large-v3 with optional INT8 + LoRA, SpecAugment, gradient ckpt."""
    model_name = config["model_name"]
    use_8bit = config.get("load_in_8bit", False)
    ...
    if use_8bit:
        ...
        load_kwargs["torch_dtype"] = (
            torch.float16 if config.get("fp16", True) else torch.float32
        )

    model = WhisperForConditionalGeneration.from_pretrained(model_name, **load_kwargs)
    processor = WhisperProcessor.from_pretrained(model_name)

    # Prepare for quantized training if using INT8
    if use_8bit:
...
def main(argv: list[str] | None = None) -> float:
    """Run Whisper-large-v3 LoRA fine-tuning.
    ...
    train_ds, val_ds = build_datasets(config, args.metadata_path, args.audio_dir)
    ...
    # Train
    logger.info("Starting LoRA training (dry_run=%s)", args.dry_run)
    trainer.train()
