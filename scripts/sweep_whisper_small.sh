#!/usr/bin/env bash
# End-to-end hyperparameter sweep for Whisper-small via Kaggle API
# Usage:
#   ./scripts/sweep_whisper_small.sh
#   ./scripts/sweep_whisper_small.sh --max-trials 5 --strategy grid
set -euo pipefail
...
OUTPUT_DIR="output/sweep/whisper-small"
POLL_INTERVAL=60

echo "=== ChildWhisper Sweep: whisper-small ==="
...
trap 'echo ""; echo "Polling stopped. Kernels still running on Kaggle."; echo "Check manually: kaggle kernels status <slug>"; exit 0' INT
